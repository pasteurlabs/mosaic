...
::: {.callout-note}
Plot styling (`color`, `linestyle`, `marker`) is **not** declared in the YAML. It lives in `mosaic/benchmarks/plots/solver_styles.py`. After adding your solver, register its style in the `SOLVER_STYLES` dict there using the solver key (typically the directory name with hyphens replaced by underscores).
:::
...
Every domain defines a shared `InputSchema` and `OutputSchema` in `mosaic/tesseract_shared/problems/<domain>/schemas.py`. Your solver imports these and can optionally subclass them to add solver-specific parameters.
...
The relative path `../../../tesseract_shared` is resolved at build time relative to your solver directory. It ensures the canonical `InputSchema` / `OutputSchema` are available inside the container.

::: {.callout-note}
For non-Python runtimes (Julia, C++, …) you'll also need `custom_build_steps` in `tesseract_config.yaml` to install the runtime. Existing Mosaic solvers show two patterns: `incompressible-navier-stokes-jl` (Julia via apt + juliacall) and `openfoam` (C++ via apt). The [pasteurlabs/tesseracts](https://github.com/pasteurlabs/tesseracts) example repo collects many more patterns — base-image overrides, system-package installation, multi-stage builds, GPU runtimes — independent of Mosaic.
:::
...
```bash
mosaic run -p ns-grid --suites gradient -s my_spectral_solver -e fd_check --no-build
```
...
Most solvers work with the default problem configuration out of the box. These overrides live entirely in `mosaic/benchmarks/problems/<domain>.py` — they're per-(solver, problem) and have no analogue in base Tesseract:
...
"my_spectral_solver": SolverSpec(
    ...
    explained_anomalies={
        "forward/cylinder": {
            "reason": "spectral pressure solve requires periodic BCs; channel runs deviate from the no-slip reference",
        },
    },
),
```
