...
::: {.callout-warning}
`mosaic run` builds ~20 Docker containers and runs the complete evaluation. Expect **several hours** on a machine with a modern GPU, or **much longer** on CPU only. Consider starting with a single problem (`mosaic run --problems thermal-mesh`) to validate your setup.
:::
