...
| `goals` | `list[Goal]` | Success predicates; each `Goal.check(result) -> bool` is evaluated after the run and the boolean lands in `result["goals"][goal.name]` |

## `Goal`

A frozen dataclass `(name: str, description: str, check: Callable[[dict], bool])`. Predicates run after the experiment completes; a raising check records `False` rather than crashing the run.
