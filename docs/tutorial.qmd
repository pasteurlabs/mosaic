...
2. **`tesseract_requirements.txt`** references the shared package with `../../tesseract_shared` (still one level up — `mosaic/tesseracts/<your-domain>/<solver>/` to `mosaic/tesseracts/tesseract_shared/`).
...
**Part A (solver to an existing domain).** Write one directory under `mosaic/tesseracts/<domain>/<solver>/` with three files (`tesseract_config.yaml`, `tesseract_api.py`, `tesseract_requirements.txt`). Subclass the canonical `InputSchema` / `OutputSchema` from `tesseract_shared`. Implement `apply` (always) and `vector_jacobian_product` (if you want gradients). Add a `metadata.mosaic:` block so the harness picks up your solver. Register plot styling in `solver_styles.py`. Add per-problem exclusions in the problem's `config.py` if needed.

**Part B (new domain).** Scaffold from `mosaic templates` to get the file tree, define canonical schemas in `tesseract_shared/problems/<domain>/`, fill in `ics.py` / `physics.py` / `experiments.py` / `config.py` under `mosaic/benchmarks/problems/<domain>/`, then add a reference solver via Part A.
