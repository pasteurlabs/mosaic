...
from mosaic.benchmarks.core.config import (
    Goal, Problem, SolverSpec, discover_solvers,
)
...
problem.add_experiment(
    ...
    goals=[
        Goal("fd_cosine_above_99pc", "max FD/VJP cosine > 0.99",
             check=lambda r: max(s.get("fd_cosine_max", 0.0)
                                 for s in r.get("by_solver", {}).values()) > 0.99),
    ],
    plot={
...
| `goals` | List of `Goal(name, description, check)` success predicates; each `check(result_dict) -> bool` is evaluated after the run and recorded under `result["goals"]`. |
| `status_check` | List of per-cell status callables (factories in `mosaic.benchmarks.core.status_checks`). |
...
Per-experiment plot descriptions live on `Experiment.params` (read via `cfg.get_plot_description(suite, exp)`); they're passed inline via `plot_description=` on each `.add_experiment(...)` call. For the per-field anatomy of `Problem` / `SolverSpec` / `Experiment`, see [Internals](internals.qmd).
