...
@pytest.mark.parametrize(
    ...
    [
        ...
        "ucurves",
        "visual_abstract",
        "conductivity_overview",
    ],
)
def test_paper_plot_generates(paper_plot, dummy_corpus, tmp_path):
    """Every paper-figure generator runs against the dummy result corpus.

    Each generator reads from ``MOSAIC_RESULTS_DIR`` (set by the
    ``dummy_corpus`` fixture) and writes a PDF/PNG into ``tmp_path``.
    Generators are expected to gracefully hide panels for missing inputs
    rather than raise — log-scale-on-zero / divide-by-zero crashes from
    the constant dummy data are converted to skips by
    :func:`_skip_if_degenerate`; anything else propagates as a real bug.
    """
    import matplotlib

    matplotlib.use("Agg")
    from mosaic.benchmarks.plots import paper

    try:
        paper.get_generate_fn(paper_plot)(tmp_path)
    except (ValueError, ZeroDivisionError) as exc:
        _skip_if_degenerate(exc)
        raise
