...
def generate(out_dir: Path) -> None:
    ...
    rec_fields = [
        _vorticity_z_slice(f[f"final_rec_all_{i}"][_STEPS_IDX], dx, z_mid)
        for i in range(n)
    ]
    ...
    fig.suptitle(
        r"Vorticity $\omega_z$ — IC and final states at steps=160  (σ=0.25, central z-slice)",
        fontsize=8.5,
        fontweight="bold",
        y=0.97,
    )
