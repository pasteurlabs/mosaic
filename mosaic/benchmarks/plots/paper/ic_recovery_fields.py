...
from mosaic.benchmarks.plots.paper.style import RCPARAMS, solver_props
...
def generate(out_dir: Path) -> None:
    ...
    for row, (si, sname) in enumerate([(best_i, _BEST), (worst_i, _WORST)]):
        ...
        _panel(ax0, vort_true, "RdBu_r", -vlim, vlim, title=titles[0], ylabel=row_label)
    ...
    for si, steps in enumerate(sweep_vals):
        ...
        last_im = _panel(
            ax, vort_rec, "RdBu_r", -vlim, vlim, title=title, ylabel=ylabel
        )
    ...
    fig.text(
        0.01,
        pos_b.y1 + 0.005,
        "(b) Observability horizon — Exponax",
        fontsize=8,
        fontweight="bold",
        va="bottom",
    )
