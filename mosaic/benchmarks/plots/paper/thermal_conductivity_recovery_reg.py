"""2D conductivity inversion (regularized): GT vs per-solver recovered rho fields.

Loads from conductivity_recovery_regularized experiment (rho_fields.npz).
Shows GT conductivity, per-solver recovered conductivity, and pointwise error
as 2D heatmaps.

Output: thermal_conductivity_recovery_reg.pdf / .png
"""
...
_EXP_DIR = (
    RESULTS / "thermal-mesh" / "optimization" / "conductivity_recovery_regularized"
)
...
def generate(out_dir: Path) -> None:
    ...
    if not npz_path.exists():
        print(
            f"[thermal_conductivity_recovery_reg] {npz_path} not found — run: "
            "mosaic recovery -p thermal-mesh -e conductivity_recovery_regularized"
        )
        return
    ...
    with plt.rc_context(RCPARAMS):
        ...
        im_gt = _imshow(
            ...
            vmin_rho,
            vmax_rho,
            title=r"GT conductivity $\rho$",
            ylabel="Ground truth",
        )
        rho_init = data["rho_init"].reshape(ny, nx)
        _imshow(
            ax_init, rho_init, "viridis", vmin_rho, vmax_rho, title=r"Init $\rho_0$"
        )
        ...
        penalty_weight = phys.get("penalty_weight", 0.0)
        ...
        fig.suptitle(
            f"Thermal conductivity inversion (regularized, $\\lambda={penalty_weight}$)",
            fontsize=8,
            y=0.97,
        )

        for ext in ("pdf", "png"):
            out = out_dir / f"thermal_conductivity_recovery_reg.{ext}"
            fig.savefig(out)
            print(f"Saved {out}")
        plt.close(fig)
