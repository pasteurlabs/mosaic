...
def generate(out_dir: Path) -> None:
    ...
    def arrow(ax, x0, y0, x1, y1, color=C_ARROW, lw=1.8):
        ax.annotate(
            ...
            xytext=(x0, y0),
            arrowprops=dict(
                arrowstyle="-|>", color=color, lw=lw, connectionstyle="arc3,rad=0"
            ),
        )
    ...
    for i, txt in enumerate(inner_items):
        ...
        box(ax, inner_x, y, inner_w, inner_h, txt, fc="white", ec=C_TESS, fontsize=6.5)
    ...
    ax.text(
        (col1_x + solver_w + 0.6 + col2_x - 0.1) / 2,
        y_start - 0.45,
        "wrap",
        ha="center",
        va="center",
    ...
    for i, txt in enumerate(eval_items):
        ...
        box(ax, item_x, y, item_w, item_h, txt, fc="white", ec=C_EVAL, fontsize=6.5)

    mid_y = tess_y + tess_h / 2
    arrow(ax, col2_x + tess_w + 0.1, mid_y, col3_x - 0.1, mid_y)

    ax.text(
        (col2_x + tess_w + 0.1 + col3_x - 0.1) / 2,
        mid_y + 0.2,
        "auto",
        ha="center",
        va="center",
    ...
    ax.text(
        ...
        **FONT,
        bbox=dict(
            boxstyle="round,pad=0.3", facecolor="#F8F8F8", edgecolor="#CCC", alpha=0.8
        ),
    )
