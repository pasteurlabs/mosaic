...
def _n_to_elements(N: int, subdir: str) -> int:
    ...
    if subdir == "structural-mesh": return N * 2 * max(1, N // 2)
    if subdir == "thermal-mesh":    return N * max(1, N // 2)
    return N


def _add_failure(ax, last_el: float, fail_el: float, last_val: float,
                 ft: str, color: str, ls: str) -> None:
    """Draw horizontal connector + failure marker from last ok point to fail N."""
    fm = _FAILURE_MARKER.get(ft, "D")
    ax.loglog([last_el, fail_el], [last_val, last_val],
              color=color, linestyle=ls, linewidth=1.0, marker="none", zorder=3)
    ax.loglog([fail_el], [last_val],
...
def generate(out_dir: Path) -> None:
    ...
    fig.subplots_adjust(left=0.07, right=0.98, bottom=0.18, top=0.97,
    ...
    for col, (domain_label, subdir, _res_key) in enumerate(DOMAINS):
        ...
        fwd_data = json.loads(fwd_path.read_text()).get("by_N", {}) if fwd_path.exists() else {}
        vjp_data = json.loads(vjp_path.read_text()).get("by_N", {}) if vjp_path.exists() else {}
        ...
        for solver in all_solvers:
            ...
            if fwd_pts:
                ...
                if fwd_fail_N is not None:
                    ...
                    _add_failure(ax_fwd, els_f[-1], fail_el, fwd_pts[ns_f[-1]],
                                 fwd_ft, color, ls)
                    all_ns.add(fail_el)
            ...
            if vjp_pts:
                ...
                if vjp_fail_N is not None:
                    ...
                    _add_failure(ax_vjp, els_v[-1], fail_el, vjp_pts[ns_v[-1]],
                                 vjp_ft, color, ls)
                    all_ns.add(fail_el)
            ...
            if fmem_pts:
                ...
                if fwd_fail_N is not None:
                    fail_el = _n_to_elements(fwd_fail_N, subdir)
                    _add_failure(ax_fmem, els_fm[-1], fail_el, fmem_pts[ns_fm[-1]],
                                 fwd_ft, color, ls)
                fmem_any = True
            ...
            if vmem_pts:
                ...
                if vjp_fail_N is not None:
                    fail_el = _n_to_elements(vjp_fail_N, subdir)
                    _add_failure(ax_vmem, els_vm[-1], fail_el, vmem_pts[ns_vm[-1]],
                                 vjp_ft, color, ls)
                vmem_any = True
    ...
    failure_handles = [
        mlines.Line2D([], [], marker=_FAILURE_MARKER[ft], color="0.4",
                      linestyle="none", markersize=7, markeredgewidth=1.0,
                      markeredgecolor="white", label=_FAILURE_LABEL[ft])
    ...
    fig.legend(handles=fem_handles,
               ...
               **legend_kw)
    if failure_handles:
        fig.legend(handles=failure_handles,
                   loc="upper center",
