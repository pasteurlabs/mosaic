...
def _status(field_data: dict) -> int:
    ...
    if s == "ok":               return OK
    if s == "not_differentiable": return NO_GRAD
    return ERROR
...
def generate(out_dir: Path) -> None:
    ...
    columns = [
        ...
        ("Structural", "structural-mesh",struct, ("input/E_max", "input/E"), "Young's\nmod."),
    ...
    domain_title_h = 0.45
    legend_h = 1.0
    coord_h = (n_solvers * cell_h + field_label_h + domain_title_h + 0.2
               + legend_h + 0.3)
    ...
    for i in range(n_solvers):
        ...
        for j in range(n_cols):
            ...
            ax.text(
                ...
                fontsize=11, fontweight="bold", color=sym_colors[val],
            )

    # ── Solver row labels (left of grid) ─────────────────────────────────────
    for i, (name, *_) in enumerate(solvers):
        row_y = (n_solvers - 1 - i) * cell_h + (cell_h - 0.06) / 2
        ax.text(x0_grid - 0.15, row_y, name,
                ha="right", va="center", fontsize=8, fontfamily="sans-serif")
    ...
    for j, (_, _, _, _, field_label) in enumerate(columns):
        ...
        ax.text(cx, label_base_y, field_label,
    ...
    for domain, (j0, j1) in domain_spans.items():
        ...
        cx = (x_left + x_right) / 2
        ax.text(cx, header_y, domain, ha="center", va="bottom",
                fontsize=9, fontweight="bold", fontfamily="sans-serif")
        ax.plot([x_left, x_right], [header_y - 0.08, header_y - 0.08],
                color="#555", linewidth=1.2, clip_on=False)
    ...
    for k, (val, label) in enumerate(legend_items):
        ...
        rect = mpatches.FancyBboxPatch(
            (lx, ly), 0.35, 0.32,
            boxstyle="round,pad=0.02",
            facecolor=colors[val], edgecolor="#ccc", linewidth=1,
        )
        ax.add_patch(rect)
        ax.text(lx + 0.52, ly + 0.16, label,
