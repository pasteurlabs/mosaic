...
def plot_source_recovery(cfg: ProblemConfig, save: bool = True, suffix: str = ""):
    ...
    The fields figure (``source_recovery_fields.{png,pdf}``) mirrors the UX of
    ``plot_lid_cavity``'s ``lid_cavity_fields`` panel but for 1-D source fields:
    a single axes overlays ``source_init`` and every solver's ``source_final_<name>``
    curve, and a second row of heatmaps shows ``source_history_<name>`` (iteration
...
def _plot_source_recovery_fields(
    ...
    *,
    ...
    from the npz render as a "no data" placeholder axis — mirrors
    ``plot_lid_cavity``'s fallback.
    """
    npz = np.load(fields_path)
    ...
    for j, name in enumerate(solver_names):
        ...
        if hkey is None:
            # "no data" fallback — match lid_cavity UX.
            ax.set_title(f"{label}\n(no data)", fontsize=8)
            ax.axis("off")
            continue
...
def _source_recovery_grid_dims(cfg: ProblemConfig) -> tuple[int, int, int, float, float]:
    ...
    ``cfg.inverse_defaults["source_recovery"]``.  Falls back to the defaults
    documented in ``thermal_mesh.py`` (nx=16, ny=8, nz=1, Lx=2.0, Ly=1.0)
    when keys are missing.  The returned values are the *configured* run
    sizes; callers must still cross-check against actual array length to
    handle ``--debug`` (which caps nx and adjusts ny).
    """
    runs = cfg.inverse_defaults.get("source_recovery", [])
    if isinstance(runs, dict):
        runs = runs.get("runs", [])
    phys: dict = {}
    if runs:
        phys = runs[0].get("physics", {}) or {}
    nx = int(phys.get("nx", 16))
    ny = int(phys.get("ny", max(1, nx // 2)))
    nz = int(phys.get("nz", 1))
    Lx = float(phys.get("Lx", 2.0))
    Ly = float(phys.get("Ly", 1.0))
    return nx, ny, nz, Lx, Ly


def _reshape_canonical_2d(
    arr: np.ndarray,
    nx: int,
    ny: int,
    nz: int,
) -> tuple[np.ndarray, str] | None:
    """Reshape a flat thermal field to ``(ny', nx')`` under the canonical layout.

    Canonical layout (matches ``_gaussian_source`` / ``_hex_mesh_arrays`` in
    ``thermal_mesh.py``):

      - Per-cell flat order is ``(nz, ny, nx)``, x innermost.
      - Per-node flat order is ``(nz+1, ny+1, nx+1)``, x innermost.

    For the quasi-2D regime (``nz==1`` configured) this produces ``(1, ny, nx)``
    or ``(2, ny+1, nx+1)`` respectively; the z axis is squeezed (we plot the
    z=0 slice) so the result is always 2D ``(rows=y, cols=x)``.

    When the configured ``nx, ny, nz`` do not multiply to the flat size we
    re-derive them from the array length under the canonical 2:1 aspect
    (nx≈2·ny, nz=1) — this handles ``--debug`` (nx capped to 8, ny=4).

    Returns ``(grid, kind)`` where ``kind`` is ``"cell"`` or ``"node"``, or
    ``None`` if no canonical reshape fits.
    """
    n = int(arr.size)

    # 1) Try the configured (nx, ny, nz) directly.
    if nx > 0 and ny > 0 and nz > 0:
        if n == nx * ny * nz:
            grid = arr.reshape(nz, ny, nx)
            return grid[0], "cell"  # squeeze z (quasi-2D plot of z=0 slice)
        if n == (nx + 1) * (ny + 1) * (nz + 1):
            grid = arr.reshape(nz + 1, ny + 1, nx + 1)
            return grid[0], "node"

    # 2) Configured dims didn't match — this happens when the npz was written
    #    by a `--debug` run (which halves nx/ny while keeping nz and aspect).
    #    Re-derive (nx_eff, ny_eff) that preserves the configured Lx:Ly aspect
    #    (= nx:ny) and the configured nz, by scanning descending nx_eff values.
    if nx > 0 and ny > 0 and nz > 0:
        # Preserve aspect: ny_eff = nx_eff * ny / nx. Iterate nx_eff downward
        # from the configured nx; require ny_eff to be an integer that also
        # yields the correct flat size under either the cell or node layout.
        for nx_eff in range(nx, 0, -1):
            if (nx_eff * ny) % nx != 0:
                continue
            ny_eff = (nx_eff * ny) // nx
            if ny_eff < 1:
                continue
            if n == nx_eff * ny_eff * nz:
                return arr.reshape(nz, ny_eff, nx_eff)[0], "cell"
            if n == (nx_eff + 1) * (ny_eff + 1) * (nz + 1):
                return (
                    arr.reshape(nz + 1, ny_eff + 1, nx_eff + 1)[0],
                    "node",
                )

    # 3) Last-ditch fallback: derive (nx', ny') under canonical 2:1 aspect,
    #    assuming nz=1. Handles the case where cfg dims are absent/zero.
    for ny_g in (
        max(1, round((n / 2) ** 0.5)),
        max(1, round((n / 2) ** 0.5) + 1),
        max(1, round((n / 2) ** 0.5) - 1),
    ):
        if ny_g <= 0 or n % ny_g != 0:
            continue
        nx_g = n // ny_g
        if 0.4 * nx_g <= 2 * ny_g <= 2.5 * nx_g:
            return arr.reshape(ny_g, nx_g), "cell"
    #    Per-node under nz=1 assumption: n = 2 · (nx'+1) · (ny'+1).
    if n % 2 == 0:
        half = n // 2
        for ny1_g in (
            max(2, round((half / 2) ** 0.5)),
            max(2, round((half / 2) ** 0.5) + 1),
            max(2, round((half / 2) ** 0.5) - 1),
        ):
            if ny1_g <= 1 or half % ny1_g != 0:
                continue
            nx1_g = half // ny1_g
            if 0.4 * nx1_g <= 2 * ny1_g <= 2.5 * nx1_g:
                return arr.reshape(2, ny1_g, nx1_g)[0], "node"
    return None


def _plot_source_recovery_thermal(
    cfg: ProblemConfig,
    npz,
    out_dir: Path,
    solver_names: list,
    ...
    *,
    ...
    """Render ``source_recovery_thermal.{png,pdf}`` — truth vs final T per solver.

    For each solver with ``temperature_final_<name>`` in the npz, draw three
    panels: truth T, recovered T, and their difference (diverging colormap).
    Uses the canonical ``(nz, ny, nx)`` per-cell / ``(nz+1, ny+1, nx+1)``
    ...
    n_solvers = len(final_names)
    n_cols = 3  # [truth, final, diff]
    fig, axes = plt.subplots(
        n_solvers,
        n_cols,
        figsize=(3.6 * n_cols, 2.8 * max(n_solvers, 1)),
        squeeze=False,
    )
    fig.suptitle(f"{cfg.name} — source recovery thermal fields")

    for i, name in enumerate(final_names):
        label = styles.get(name, {}).get("label", name)
        T_final = np.asarray(npz[f"temperature_final_{name}"])

        # Choose the truth to compare against.  Prefer the global truth when
        # shapes match; else the per-solver truth; else fall back to skipping.
        if T_truth is not None and T_truth.shape == T_final.shape:
            T_gt = T_truth
            gt_source = "reference"
        elif f"temperature_truth_{name}" in npz.files:
            T_gt = np.asarray(npz[f"temperature_truth_{name}"])
            gt_source = name
        else:
            T_gt = None
            gt_source = None

        ax_truth, ax_final, ax_diff = axes[i]

        reshape_final = _reshape_canonical_2d(T_final, nx, ny, nz)
        reshape_gt = (
            _reshape_canonical_2d(T_gt, nx, ny, nz) if T_gt is not None else None
        )
        grid_final = reshape_final[0] if reshape_final is not None else None
        grid_gt = reshape_gt[0] if reshape_gt is not None else None

        if (
            grid_gt is not None
            and grid_final is not None
            and grid_gt.shape == grid_final.shape
        ):
            # Canonical 2D layout: rows → y, cols → x. No transpose, origin lower,
            # extent in physical units so axes labels are meaningful.
            vmin = float(min(grid_gt.min(), grid_final.min()))
            vmax = float(max(grid_gt.max(), grid_final.max()))
            ...
            ax_truth.set_title(f"truth T ({gt_source})", fontsize=8)
            plt.colorbar(im0, ax=ax_truth, fraction=0.04)
            im1 = ax_final.imshow(
                grid_final,
                origin="lower",
                cmap="hot",
            ...
            ax_final.set_title(f"{label} — final T", fontsize=8)
            plt.colorbar(im1, ax=ax_final, fraction=0.04)
            diff = grid_final - grid_gt
            ...
            ax_diff.set_title(f"{label} − truth", fontsize=8)
            plt.colorbar(im2, ax=ax_diff, fraction=0.04)
            for ax in (ax_truth, ax_final, ax_diff):
                ax.set_xlabel("x")
                ax.set_ylabel("y")
        else:
            # Shape didn't match the canonical (nz, ny, nx) / (nz+1, ny+1, nx+1)
            # layout for either field — fall back to side-by-side line plots so
            # the figure still renders something useful.
            xs_final = np.arange(T_final.size)
            if T_gt is not None:
                xs_gt = np.arange(T_gt.size)
                ax_truth.plot(xs_gt, T_gt, color="black", linewidth=1.2)
            ax_truth.set_title(f"truth T ({gt_source or '—'})", fontsize=8)
            ax_truth.grid(True, alpha=0.3)
            color = styles.get(name, {}).get("color", "#3366CC")
            ax_final.plot(xs_final, T_final, color=color, linewidth=1.3)
            ax_final.set_title(f"{label} — final T", fontsize=8)
            ax_final.grid(True, alpha=0.3)
            if T_gt is not None and T_gt.shape == T_final.shape:
                err = T_final - T_gt
                ax_diff.axhline(0.0, color="black", linewidth=0.6, alpha=0.4)
                ax_diff.plot(xs_final, err, color=color, linewidth=1.3)
                emax = float(np.abs(err).max()) or 1.0
                ax_diff.set_ylim(-1.05 * emax, 1.05 * emax)
                ax_diff.set_title(f"{label} − truth", fontsize=8)
                ax_diff.grid(True, alpha=0.3)
            else:
                ax_diff.axis("off")
                ax_diff.set_title(f"{label}\n(shape mismatch)", fontsize=8)
...
def plot_lid_cavity(cfg: ProblemConfig, save: bool = True, suffix: str = "", exp_key: str = "lid_cavity") -> None:
    ...
    # Collect all sweep values from first solver's keys
    _first = next(iter(by_sweep.values()), {})
    sweep_vals = sorted(
        _first.keys(),
        key=lambda v: float(v) if str(v).replace(".", "").lstrip("-").isdigit() else 0,
    )
    # Exclude solvers that are categorically excluded for this experiment (e.g. exponax:
    ...
    # Drop sweep values where every solver's loss is identically zero (trivial
    # case — initial guess already matches target, nothing to optimise).
    # Panels would render as empty axes otherwise.
    def _is_trivial(v) -> bool:
        for name in solver_names:
            r = by_sweep[name].get(v) or by_sweep[name].get(str(v))
    ...
    non_trivial_vals = [v for v in sweep_vals if not _is_trivial(v)]
    ...
    if non_trivial_vals:
        ...
        for ax, v in zip(axes_lc, non_trivial_vals):
            for name in solver_names:
                ...
                r = s_results.get(v) or s_results.get(str(v))
                if isinstance(r, dict) and r.get("losses"):
                    sty = styles.get(name, {})
                    ax.semilogy(
                        r["losses"],
                        label=sty.get("label", name),
            ...
            ax.set_xlabel("Iteration")
            ax.set_title(f"U_x_true = {v}")
        axes_lc[0].set_ylabel("MSE loss (mid-plane)")
        fig_lc.suptitle(f"{cfg.name} — lid_cavity convergence curves")
        fig_shared_legend(fig_lc, axes_lc)
        if save:
            save_fig(fig_lc, "lid_cavity_convergence", out_dir)

    # ── Final loss bar chart across non-trivial sweep values (log-y) ─────────
    # Final losses span many decades (1e-8 to 1e-3) so a linear axis hides
    # all but the largest bars. Log-y + a small epsilon floor for bars that
    # are exactly zero keeps every solver visible.
    if non_trivial_vals:
        x = np.arange(len(non_trivial_vals))
        width = 0.8 / max(len(solver_names), 1)

        fig_bar, ax_bar = plt.subplots(figsize=(max(8, len(non_trivial_vals) * 2.5), 4))
        # Compute an epsilon just below the smallest positive final loss so
        # zero / non-finite bars render as a short stub instead of vanishing
        # on the log axis.
        all_finite_pos: list[float] = []
        for name in solver_names:
            s_results = by_sweep[name]
            for v in non_trivial_vals:
                r = s_results.get(v) or s_results.get(str(v))
                if isinstance(r, dict):
                    fl = r.get("final_loss")
                    if isinstance(fl, (int, float)) and math.isfinite(fl) and fl > 0:
                        all_finite_pos.append(float(fl))
        eps_floor = min(all_finite_pos) * 0.1 if all_finite_pos else 1e-12
        ...
        for i, name in enumerate(solver_names):
            ...
            for v in non_trivial_vals:
                ...
                if isinstance(r, dict):
                    ...
                    fl_f = float(fl) if isinstance(fl, (int, float)) else float("nan")
                    if math.isfinite(fl_f) and fl_f > 0:
                        final_losses.append(fl_f)
                    else:
                        final_losses.append(eps_floor)
                else:
                    final_losses.append(eps_floor)
            sty = styles.get(name, {})
            color = cfg.solvers[name].color if name in cfg.solvers else "#888888"
            ax_bar.bar(
                x + i * width,
                final_losses,
                width,
        ...
        ax_bar.set_ylim(bottom=eps_floor * 0.5)
        ax_bar.set_xticks(x + width * len(solver_names) / 2)
        ax_bar.set_xticklabels([f"U_x={v}" for v in non_trivial_vals])
        ax_bar.set_ylabel("Final MSE loss (log scale)")
        ax_bar.set_title(f"{cfg.name} — lid_cavity final loss per U_x_true")
        # set_axisbelow puts gridlines behind the bars so the minor-grid
        # striping of a log axis doesn't render as horizontal lines through
        # the bar fills.
        ax_bar.set_axisbelow(True)
        ax_bar.grid(axis="y", which="major")
        ax_bar.grid(False, axis="x")
        fig_shared_legend(fig_bar, [ax_bar])
        if save:
            save_fig(fig_bar, "lid_cavity_final_loss", out_dir)

    # ── Optimised lid field visualisation (if npz exists) ────────────────────
    fields_path = out_dir / "lid_fields.npz"
    if not fields_path.exists():
        return

    npz = np.load(fields_path)
    if "lid_init" not in npz:
        return
    lid_init = npz["lid_init"]

    params = data.get("params", {})
    sweep_vals_params = params.get("sweep", {}).get("values", [])
    U_x_ic = float(params.get("ic", {}).get("U_x", 0.5))
    non_trivial_u = [v for v in sweep_vals_params if float(v) != U_x_ic]
    boundary_pad = int(params.get("physics", {}).get("boundary_pad", 0))

    # Only show solvers that actually ran (non-empty by_sweep with non-trivial
    # initial_loss > 0).  OOM-excluded solvers like phiflow may appear in
    # by_sweep.keys() with an empty dict, but their stale NPZ data would render
    # a misleading constant field from a prior run.
    def _has_nontrivial_data(name: str) -> bool:
        return any(
            isinstance(v, dict)
            and isinstance(v.get("initial_loss"), (int, float))
    ...
    # consistent across all per-U figures.
    vmax_candidates = [float(np.abs(lid_init[:, :, 0]).max())]
    if non_trivial_u:
        vmax_candidates.append(float(max(non_trivial_u, key=float)))
    vmax = max(vmax_candidates) * 1.1 + 1e-6

    def _u_tag(u_val) -> str:
        return str(int(u_val)) if float(u_val).is_integer() else f"{u_val:g}"

    def _get_opt_field(name: str, u_t: str) -> np.ndarray | None:
        """Last frame of history for (solver, U); fall back to lid_opt_{name}."""
        hist_key = f"lid_history_{name}_{u_t}"
        if hist_key in npz.files:
            hist = np.asarray(npz[hist_key])
            if hist.ndim == 4 and hist.shape[0] > 0:
                return hist[-1]
        opt_key = f"lid_opt_{name}"
        return np.asarray(npz[opt_key]) if opt_key in npz.files else None
    ...
    # Determine ic_mode for spatial experiments (lid_cavity_spatial uses rotational targets)
    ic_mode = params.get("ic", {}).get("mode", None)

    # One fields figure per non-trivial U value (Init | True | opt... / residuals)
    for u_val in non_trivial_u:
        u_t = _u_tag(u_val)
        # Use saved spatial true field if available (lid_cavity_spatial), else uniform fallback
        true_key = f"lid_true_{u_t}"
        if true_key in npz.files:
            lid_true = np.asarray(npz[true_key])
        else:
            lid_true = np.zeros_like(lid_init)
            lid_true[:, :, 0] = float(u_val)

        n_fld_solvers = len(fld_solver_names)
        ...
        for j, name in enumerate(fld_solver_names):
            label = styles.get(name, {}).get("label", name)
            ax_top = axes_fld[0, j + 2]
        ...
        suptitle_mode = f" ({ic_mode})" if ic_mode else ""
        fig_fld.suptitle(
        ...
        fig_fld.tight_layout()
        if save:
            save_fig(fig_fld, f"lid_cavity_fields_{u_t}", out_dir)
...
def _render_lid_cavity_evolution_gifs(
    ...
    """Write ``lid_cavity_evolution_<solver>_<U>.gif`` per (solver, sweep).

    Each frame shows the u_x component of the optimised lid field at one
    snapshot; ``U_x_true`` is preserved in the filename so every sweep value
    gets its own animation.  Silently skips keys that do not match the
    ``lid_history_<solver>_<U>`` template.
    """
    for key in npz.files:
        if not key.startswith("lid_history_"):
            continue
        hist = np.asarray(npz[key])  # (n_snaps, N, N, 2)
        if hist.ndim != 4 or hist.shape[0] == 0:
            continue
        remainder = key[len("lid_history_") :]
        # Split from the right so solver names with underscores are preserved.
        if "_" not in remainder:
            continue
        sname, u_tag = remainder.rsplit("_", 1)
        if sname not in solver_names:
            # Skip stale NPZ entries from prior runs (e.g. OOM-excluded solvers).
            continue
        n_frames = int(hist.shape[0])
        label = styles.get(sname, {}).get("label", sname)
        ...
        title = ax.set_title(
            f"{label} U_x={u_tag} — snapshot 1 / {n_frames}", fontsize=9
        )
        ax.axis("off")
...
def _render_lid_cavity_combined_gifs(
    ...
    """Write ``lid_cavity_evolution_combined_<U>.gif`` — all solvers side-by-side.

    For each non-trivial U_x_true value, creates a single animated figure with
    ...
    for u_val in non_trivial_u:
        u_t = _u_tag(u_val)
        # Collect histories for this U value (only solvers with data).
        histories = []
        labels = []
        for name in solver_names:
            key = f"lid_history_{name}_{u_t}"
            if key not in npz.files:
                continue
            ...
            labels.append(styles.get(name, {}).get("label", name))

        if not histories:
            continue
        n_frames = max(h.shape[0] for h in histories)
        if n_frames < 2:
            continue  # nothing to animate — skip trivial sweep values

        n_cols = len(histories)
        fig, axes = plt.subplots(1, n_cols, figsize=(4.0 * n_cols, 4.0), squeeze=False)
        axes = axes[0]
        ...
        def _update(idx, _ims=ims, _titles=titles, _hists=histories, _labels=labels, _n=n_frames):
            ...
            for im_, title_, hist_, label_ in zip(_ims, _titles, _hists, _labels):
                ...
                im_.set_data(hist_[frame_idx, :, :, 0].T)
                title_.set_text(f"{label_}\nstep {idx + 1}/{_n}")
                artists += [im_, title_]
            return artists
