...
def plot_differentiability_table(
    cfg: ProblemConfig, save: bool = True, suffix: str = ""
):
    """Heatmap of differentiability status for all (solver, field) pairs.

    Color encoding:
        green  = ok (FD-consistent gradient)
        red    = fail (VJP vs FD disagree by >5%)
        grey   = not_differentiable (integer / scalar / non-array input)
    ...
    # Collect all field names from union across solvers, keeping only those
    # where at least one solver reports status "ok".
    all_fields_ordered: list[str] = []
    seen: set[str] = set()
    for fields in by_solver.values():
        for f in fields:
            if f not in seen:
    ...
    all_fields = [
        ...
        if any(by_solver[s].get(f, {}).get("status") == "ok" for s in by_solver)
    ]
    # Fall back to all fields when nothing passes (e.g. all harness_opaque/not_differentiable)
    if not all_fields:
        all_fields = all_fields_ordered
    solver_names = list(by_solver.keys())

    _STATUS_COLOR = {
        "ok": "#4daf4a",
        "fail": "#e41a1c",
        "not_differentiable": "#cccccc",
        "error": "#ff7f00",
    }

    n_fields = len(all_fields)
    n_solvers = len(solver_names)
    if n_fields == 0 or n_solvers == 0:
        return None
    fig, ax = plt.subplots(figsize=(max(4, 1.8 * n_solvers), max(3, 0.45 * n_fields)))

    color_matrix = np.zeros((n_fields, n_solvers, 4))
    for j, name in enumerate(solver_names):
        fields = by_solver.get(name, {})
        for i, fname in enumerate(all_fields):
            info = fields.get(fname, {})
            status = info.get("status", "error")
            rgba = plt.matplotlib.colors.to_rgba(_STATUS_COLOR.get(status, "#888888"))
            color_matrix[i, j] = rgba

    ax.imshow(color_matrix, aspect="auto", interpolation="none")
    # Remove grid lines that bleed through the heatmap cells (axes.grid is True globally)
    ax.grid(False)

    # Annotate cells with relative error
    for j, name in enumerate(solver_names):
        fields = by_solver.get(name, {})
        for i, fname in enumerate(all_fields):
            info = fields.get(fname, {})
            rel = info.get("rel_error")
            if rel is not None:
                ax.text(
                    j,
                    i,
    ...
    solver_labels = [styles.get(n, {}).get("label", n) for n in solver_names]
    # Place solver labels above the heatmap so they don't overlap field-name rows
    ax.set_xticks(range(n_solvers))
    ax.xaxis.set_ticks_position("top")
    ax.xaxis.set_label_position("top")
    ax.set_xticklabels(solver_labels, rotation=30, ha="left", fontsize=9)
    ax.set_yticks(range(n_fields))
    ax.set_yticklabels(all_fields, fontsize=8)
    ax.set_title(f"{cfg.name} — differentiability table", pad=40)

    from matplotlib.patches import Patch

    legend_elements = [Patch(facecolor=c, label=s) for s, c in _STATUS_COLOR.items()]
    ax.legend(handles=legend_elements, loc="lower right", fontsize=8, framealpha=0.9)

    fig.tight_layout()
    if save:
        save_fig(fig, "differentiability_table", out_dir)
    return fig
